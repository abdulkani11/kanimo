...
// Server-Side AI travel companion via Gemini API
app.post('/api/ai/assistant', async (req, res) => {
  ...
  try {
    ...
    const systemInstruction = `
      You are Noble, an elite AI Concierge for a luxury travel agency and ticket portal.
      Provide highly professional, polite, and detailed flight support, travel guides, packing lists, and local recommendations.
      
      When the user asks for a travel itinerary, suggest local highlights, fine dining, and cultural secrets.
      ALWAYS format itineraries cleanly using Markdown with header levels (e.g. ### Day 1: ...) and lists.
      Maintain a sophisticated, welcoming, and knowledgeable tone. Talk like a premier travel expert.
      Do not mention technical configurations, database details, or state variables. Keep conversations strictly focused on luxury travel services.
    `;
    ...
    const reply = response.text || "I apologize, I am unable to process your travel inquiry at this moment.";
    res.json({ reply });
  } catch (err: any) {
    console.error('Error in Gemini Assistant API:', err);
    res.status(500).json({
      error: 'Unable to contact travel concierge.',
      details: err.message,
    });
  }
});

// GDS Ticket Data Extraction via Gemini API
app.post('/api/ai/extract-ticket', async (req, res) => {
  const { text, image, imageType } = req.body;
  if (!text && !image) {
    return res.status(400).json({ error: 'Text or image data is required for ticket extraction' });
  }
  ...
  try {
    ...
    const systemInstruction = `
      You are an expert travel ERP data extraction system. Your task is to extract flight pricing details from raw GDS/airline ticket strings or images and output structured JSON mapped directly to the Ticket Create module.

      ### MATHEMATICAL CALCULATION RULES
      1. NET = BASE_FARE + TAX
      2. TAX = TFC_TOTAL
      3. COMMISSION = BASE_FARE * (VENDOR_COMM_PERCENT / 100)
      4. CUST_COMM = BASE_FARE * (CUST_COMM_PERCENT / 100)
      5. TOTAL_AMOUNT_AFTER_DISCOUNT = (NET - COMMISSION + CUST_COMM - DISCOUNT) - REFUND

      ### EXTRACTION & MAPPING RULES
      - Extract BASE FARE as 'fare'.
      - Extract TFC TOTAL (Taxes, Fees & Charges) as 'tax'.
      - Compute 'net' = 'fare' + 'tax'.
      - Set default values for 'refund' and 'discount' to 0.00 unless explicitly stated otherwise.
      - Apply default commission percentages:
        - Default Customer Commission % = 6.0
        - Default Vendor Commission % = 9.0

      Also extract other rich flight information from GDS text or image to help fill out a Ticket Create form, including:
      - PNR (6 character alpha-numeric code, e.g. QZSDXG)
      - Ticket Number (e.g. 07121619 or standard 10-13 digit formats like 071-2161928371)
      - Flight Origin (3 letter airport code, e.g. MGQ, DXB)
      - Flight Destination (3 letter airport code, e.g. ADD, LHR)
      - Airline Carrier Name (e.g. Emirates, Turkish Airlines, Ethiopian Airlines, Qatar Airways, FlyDubai)
      - Departure Date (convert to YYYY-MM-DD format if possible, or extract as string)
      - Return Date (convert to YYYY-MM-DD format if possible, or extract as string)
      - Passengers: name, type (Adult/Child/Infant), passport number, nationality, date of birth (YYYY-MM-DD), gender, seat preference, meal preference.
      ...
      You MUST respond ONLY with a valid JSON object matching this structure:
      {
        "ticket_info": {
          "currency": "USD",
          "customer_commission_percent": 6.0,
          "vendor_commission_percent": 9.0
        },
        "passenger_line_items": [
          {
    ...
    if (image) {
      ...
      let mimeType = imageType || 'image/png';
    ...
    if (text) {
      parts.push({
        text: `Here is the raw GDS ticket input:\n${text}`
      });
    } else {
      parts.push({
        text: "Please extract the GDS flight ticket and pricing details from this ticket image."
      });
    }
    ...
    if (!replyText) {
      throw new Error("No response received from the extraction model.");
    }

    const parsedData = JSON.parse(replyText.trim());
    res.json(parsedData);
  } catch (err: any) {
    console.error('Error in GDS extraction endpoint:', err);
    res.status(500).json({
      error: 'Failed to extract GDS ticket data',
      details: err.message
    });
  }
});

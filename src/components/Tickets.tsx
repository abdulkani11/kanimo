...
export default function Tickets({ userRole = 'admin', loggedInEmail = 'admin@noble.com' }: TicketsProps) {
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (res.ok) {
          alert(`Successfully imported ${json.length} invoices!`);
          e.target.value = '';
  ...
  // Void/Delete ticket invoice
  const handleDeleteInvoice = async (id: string, invoicePnr: string) => {
    if (!confirm(`Are you sure you want to void and delete invoice ${id} with PNR ${invoicePnr}?`)) return;
      ...
      if (res.ok) {
        ...
        await fetch('/api/audit-logs', {
          ...
          body: JSON.stringify({
            ...
            role: 'Super Admin',
            action: 'Void Invoice',
            details: `Voided and deleted ticket invoice ${id} with PNR ${invoicePnr}`,
          }),
        });
        fetchData();
      }
